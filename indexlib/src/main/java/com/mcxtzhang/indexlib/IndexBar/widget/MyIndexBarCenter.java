 ...
 * 固定宽高 靠上
 * <p>
 * 1 mGapHeight必须是给定的值 不能变化  暂定24px 索引字为16px
 * 2 绘制之前要算好绘制的位置
 * 比如 就剩3个Index 那么算出他需要的空间大小
 * 通过这个确定总高度
 */

public class MyIndexBarCenter extends View {
    ...
    //每个index区域的高度
    // FIXME: 2017/11/18 第一步 间距肯定是指定的值
    private int mGapHeight = dp2px(14);
    ...
    public MyIndexBarCenter(Context context) {
        this(context, null);
    }

    public MyIndexBarCenter(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyIndexBarCenter(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @return
     */
    public MyIndexBarCenter setHeaderViewCount(int headerViewCount) {
     ...
     * @return
     */
    public MyIndexBarCenter setSourceDatasAlreadySorted(boolean sourceDatasAlreadySorted) {
     ...
     * @return
     */
    public MyIndexBarCenter setDataHelper(IIndexBarDataHelper dataHelper) {
...
//        int t = getPaddingTop();//top的基准点(支持padding)
        ...
        for (int i = 0; i < mIndexDatas.size(); i++) {
            ...
            canvas.drawText(index,
                    ...
                    startSize + mGapHeight * i + baseline,
    ...
    public int getStartSize(){
        ...
        int mTotalSize = mIndexDatas.size() * mGapHeight;//索引的总高度
        int startSize = (height - mTotalSize) / 2;
        return startSize;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                float y = event.getY();
                //通过计算判断落点在哪个区域：
                int pressI = (int) ((y - getStartSize()) / mGapHeight);
    ...
    public MyIndexBarCenter setmPressedShowTextView(TextView mPressedShowTextView) {
        this.mPressedShowTextView = mPressedShowTextView;
        return this;
    }

    public MyIndexBarCenter setmLayoutManager(LinearLayoutManager mLayoutManager) {
     ...
     * @return
     */
    public MyIndexBarCenter setNeedRealIndex(boolean needRealIndex) {
    ...
    public MyIndexBarCenter setmSourceDatas(List<? extends BaseIndexPinyinBean> mSourceDatas) {
